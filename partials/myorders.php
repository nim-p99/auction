<?php
// Header, Footer, and Utilities are loaded by the parent page.

$buyerID = $_SESSION['buyer_id'] ?? null;

if (!$buyerID) {
    echo "<div class='alert alert-info'>You must be logged in as a buyer to view orders.</div>";
    return;
}

// 1. Determine Context
$current_page_url = basename($_SERVER['PHP_SELF']); 
$qs_prefix = ($current_page_url == 'buyer.php') ? 'tab=myorders&' : 'section=buyer&tab=myorders&';
...
<form method="get" action="<?php echo htmlspecialchars($current_page_url); ?>">
  ...
  <?php endif; ?>
  <input type="hidden" name="tab" value="myorders">
...
<?php 
  // We join 'transaction' to ensure the auction was actually won
  $final_query = "
    SELECT 
  ...
  if (mysqli_num_rows($auctions_to_list) > 0) { 
    ...
    echo "<div class='alert alert-info'>No completed orders found.</div>";
  }
